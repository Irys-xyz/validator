...
use diesel_async::{deadpool::{ConnectionManager, ManagedAsyncConnection, Pool}, AsyncPgConnection};

use crate::{server::routes::sign::sign, database::{schema::transactions, models::Transaction}};
...
pub async fn run_server() -> std::io::Result<()> {
    ...
    let postgres_pool = Pool::builder(conn_manager)
            .max_size(10)
            .build()
            .unwrap();

    let mut conn = postgres_pool.get()
        .await
        .unwrap();

    let x = transactions::table
        .first::<Transaction>(&mut conn)
        .await
        .unwrap();

    HttpServer::new(move || {
        let conn_manager = ConnectionManager::<AsyncPgConnection>::new(db_url.clone());
