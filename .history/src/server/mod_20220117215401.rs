...
use diesel_async::deadpool::{ConnectionManager};
use paris::info;
use reool::RedisPool;
...
pub async fn run_server() -> std::io::Result<()> {
    ...
    let manager = ConnectionManager::<diesel_async::AsyncPgConnection>::new(db_url);

    let p = diesel_async::deadpool::Pool::builder(manager)
        .build()
        .unwrap();
