...
const BUNDLER_ADDRESS: String = BASE64URL.encode(BUNDLER_PUBLIC);
...
async fn verify_body(body: &UnsignedBody) -> bool {
    let block = body.block.to_le_bytes().ma
    let message = deep_hash(DeepHashChunk::Chunks(vec![
        ...
        DeepHashChunk::Chunk(BASE64URL.decode(body.id.as_bytes()).unwrap().into()),
        DeepHashChunk::Chunk(BASE64URL.decode(body.block..as_bytes()).unwrap().into())
    ])).await.unwrap();
