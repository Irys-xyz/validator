...
pub async fn post_tx(
    body: Json<PostTxBody>, redis_client: Data<redis::Client>, awc_client: Data<awc::Client>) -> actix_web::Result<HttpResponse, ValidatorServerError> {
    ...
    if number_of_sigs < 3 {
        let validators = serde_json::from_str::<Vec<String>>("").unwrap();
        for validator in validators {
           let mut response = awc_client
            .post(format!("{}/tx", validator))
            .send_json(&body2)
            .await
            .unwrap();
