...
async fn create_cron<T: Send + 'static>(description: &'static str, fut: impl Future<Output=T> + Send + 'static, sleep: u64) {
    tokio::task::spawn(async move {
        loop {
            if let Err(e) = update_contract().await {
                error!("Error occurred while {} - {}", description.clone(), e);
                panic!("{}", e);
            };
